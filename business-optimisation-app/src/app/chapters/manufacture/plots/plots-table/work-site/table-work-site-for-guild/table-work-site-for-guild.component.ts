...
export class TableWorkSiteForGuildComponent implements OnInit {
  ...
  @Input() set isOpenTableForWorkSiteOfGuild(value: boolean) { this.canOpenTable = value; }
  ...
  ngOnInit() {
    this.getWorkSitesByStorage(StorageKey.WorkSiteForGuild);
  }
  ...
  private onSearch(): void {
    if (this.workSites != null) {
      this.updateDataService.setWorkSiteForGuild();
      this.changeableWorkSites = [];
  ...
  private rewriteStorage(): void {
    this.storage.deleteData(StorageKey.WorkSiteForGuild);
    this.workSites.forEach(item => {
      this.storage.addData(StorageKey.WorkSiteForGuild, item);
    });
  }
